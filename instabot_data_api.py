...
class registeredTablesDefinitions:
...
class dataAPI(registeredTablesDefinitions):
    
    def __init__(self):
        pass
        
    def get(self,name,user):
        if name not in [i for i in dir(registeredTablesDefinitions) if not callable(i)]:
            raise Exception("DataAPITableDoesntExist")
        exists = os.path.isfile(name+"_"+user+".csv")
        if exists:
            data = pd.read_csv(name+"_"+user+".csv")
        else:
            schema = getattr(registeredTablesDefinitions, name).get("schema")
